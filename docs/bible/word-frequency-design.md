...
| `BibleWordTokenizer` / `BibleWordMatcher` | 완료 (단위 31건 + 골든 3건 통과) |
| 관리자 API·화면 4개 | 완료 |
...
| 운영 어휘 — 후보 일괄 등록 | **롤백** (2026-08-24) — 705건을 검수 없이 등록해 통계가 망가졌다. 전량 삭제(어휘 705 + 통계 3,414행). 경위는 §7.4 의 경고 참조 |
| 운영 통계 — KRV 창세기 | **재계산 필요** — 현재 870행은 APPROVED 350건 기준이고 **수정 전 규칙으로 계산된 값**이다. 배포 후 다시 돌려야 `몸`·`떼`·`롯` 이 맞게 잡힌다 |
| CANDIDATE 노출 정책 | **끔** (2026-08-24) — 기본값 `false`. 검수를 등록 시점으로 옮겼다(§3.2) |
| 후보 화면 선택 등록·선택 차단 | 완료 (2026-08-24) — 체크박스로 골라 `APPROVED`/`BLOCKED` 로 넣는다 |
| **운영 후보 등록·재계산 (§7.4 의 3~5번)** | **재실행 필요** — 배포 후 후보를 다시 뽑아 **명사만 골라 승인 등록**한다 |
...
| `CANDIDATE` | 본문에서 자동 추출된 미검수 어휘 | 정책 스위치로 결정. **기본값 `false`(숨김)** |
| `BLOCKED` | 관리자가 영구 제외 (`나뉘게`, `하려` 같은 활용형 잔재) | 절대 안 나옴. **재추출해도 다시 후보로 올라오지 않는다** |

**검수는 등록 시점에 끝낸다.**

1. 사전에서 가져온다(APPROVED).
2. 후보 화면에서 **골라서** `APPROVED` 로 등록한다. 나머지는 선택 차단으로 쓸어 담는다.
3. 통계 화면을 보다가 이상한 단어가 있으면 `BLOCKED` 로 내린다.

**`BLOCKED` 가 재추출을 막는다는 점이 중요하다.** 이 장치가 없으면 재계산할 때마다 같은 쓰레기
단어가 되살아나 같은 작업을 반복하게 된다.

> **원래 계획은 "일단 노출하고 관리자가 쳐낸다" 였다. 그 계획은 성립하지 않았다.**
>
> 2026-08-24, 스위치를 켠 채로 후보 705건을 검수 없이 등록했더니 화면에 `하라`·`몸에`·
> `하였으므` 가 올라왔다. 문제는 쳐내는 수단이다. 상태 변경은 `PATCH /words/{id}/status` 로
> **한 건씩**뿐이라 쌓이는 속도를 따라갈 수 없다. 6천 건 규모에서는 애초에 불가능하다.
>
> 그래서 기본값을 `false` 로 바꾸고, 후보 화면에 **체크박스 선택 → 승인 등록 / 선택 차단** 을
> 넣었다. 어휘를 키우는 주 경로는 이제 "쓸어 담고 나중에 쳐내기" 가 아니라 "골라 넣기" 다.
> 빈도 조건 일괄 등록은 남아 있지만 `CANDIDATE` 로만 들어가므로 화면에 새지 않는다.
>
> 롱테일이 필요하면 스위치를 켜는 게 아니라 **승인 어휘를 키우는 쪽**으로 푼다. 정말 켜야
> 하면 `WORD_STATS_INCLUDE_CANDIDATE=true` 로 되돌릴 수 있다.
...
#### 여러 어절로 된 표제어는 매칭되지 않는다 — 알려진 한계, 그대로 둔다

조회 단위가 **어절 하나**라 표제어에 공백이 있으면 어떤 책에서도 맞지 않는다. 성경 사전에서
가져온 어휘 중 17건이 여기 해당한다.

```
하나님 나라   거룩한 백성   남은 자   기름 부음   왕 같은 제사장   새 하늘과 새 땅
빛의 자녀   하나님의 형상   향기로운 제물   영적 예배   영적 전쟁   집 짓기   길 잃음   …
```

2026-08-25 확인 시점에 개역한글·개역개정 각각 17건이고 **통계 행은 0건**이다. 오류가 아니라
설계상 당연한 결과다 — 어절 하나씩 해시로 조회하는 구조에서 두 어절짜리 키는 만들어지지 않는다.

**그대로 둔다.** 성경 사전 화면에서는 뜻풀이가 제 역할을 하고, 통계에만 안 잡힐 뿐이다.
승인 어휘 수를 정직하게 맞추려고 `BLOCKED` 로 돌리는 방법도 있지만, 차단은 "이 낱말은 표제어가
아니다" 라는 뜻이라 의미가 어긋난다.

매칭이 필요해지면 n-gram 조회를 더해야 한다 — 어절 하나뿐 아니라 인접한 2~3어절을 묶어
해시를 한 번 더 보는 방식이다. 어휘 대부분이 한 어절이라 비용 대비 이득이 작아 하지 않았다.
...
4. 서술어 어미로 끝나면 버림          (이르시되 → 버림)
   2음절은 접미사가 아니라 **어미 목록과 정확히 일치**할 때만 버림 (하라 → 버림 / 사라 → 유지)
...
몸 떼 딸 돈 날 세 굴 목 향 궁 귀 악 줄 짐 칼 삯 형 낯 비 롯 셈 단          (85건)
```
...
**빠뜨리면 그 단어가 안 나오는 게 아니라 조사 개수만큼 갈라진다.** 줄기가 1음절이면 길이 가드에
걸려 어절이 그대로 남고, 조사마다 다른 어절이 되기 때문이다. 창세기 전체 실측(2026-08-24):

| 단어 | 그때 상태 | 집계 결과 |
|---|---|---|
| 롯 | 목록·어휘에 모두 없음 | 롯이 12 / 롯을 6 / 롯의 5 / 롯도 4 / 롯과 3 — **한 인물이 표제어 5개** |
| 몸 | 어휘엔 APPROVED, 목록엔 없음 | 몸을 7 / 몸에 4 / 몸과 3, 그런데 **`몸` 자체는 0회** |
| 떼 | 어휘엔 APPROVED, 목록엔 없음 | 떼가 7 / 떼를 5 / 떼로 3, `떼` 는 1회 |
| 딸 | 목록·어휘에 모두 없음 | 딸과 / 딸을 / 딸의 / 딸이 로 4분할 |

어근 19개가 이렇게 갈라져 표제어 53개를 차지했다. 창세기의 주요 인물인 롯이 통계 어디에도
없었던 이유가 이것이다.

> **두 목록은 코드로 연결돼 있지 않다.** 토크나이저는 DB 를 보지 않는 순수 함수라
> `bible_word` 를 참조할 수 없다(§6). 그래서 **어휘에 1음절 표제어를 넣을 때는
> `one-char-nouns-ko.txt` 도 같이 봐야 한다.** `몸`·`떼` 가 승인 어휘인데 0회였던 것이 바로 이
> 어긋남이다. 형태소 분석기로 갈아탈 때 함께 정리될 부채다.

반대로 함부로 넣으면 멀쩡한 2음절 명사가 깎인다. 넣기 전에 "그 글자로 끝나는 2음절 단어 +
조사" 가 실재하는지 확인한다. 그래서 일부러 뺀 것들:

| 글자 | 빼는 이유 |
|---|---|
| 실 | `실과`(열매)가 `실` 로 깎인다 |
| 태 | `태도` 가 `태` 로 깎인다 |
| 함 | 함(노아의 아들)과 `하다` 의 명사형이 섞인다 |
| 들 | 들(들판) / `듣다` 활용형 / 복수 접미사가 겹친다 |

`풀`·`뭍` 은 창세기 1장 시범 집계에서 같은 방식으로 관측해 추가했다
(`BibleWordTokenizerTest.keepPlantAndDryLandNouns`,
`oneCharNounNoLongerSplits`, `deliberatelyExcludedOneCharNouns`).
...
**2음절 어절은 3음절 목록과 정확히 일치하는지도 본다.** 접미사 검사만 하면
`하라`·`하매`·`하여`·`하사`·`하신`·`하실`·`하기` 일곱 개가 빠져나간다. 일곱 개 모두 3음절
목록에 그대로 들어 있는데, 그 목록이 3음절 이상에만 적용되기 때문이다. 2026-08-24 운영 후보
추출에서 전부 어휘로 올라가 `하라(3회)` 처럼 화면에 노출됐다.

고치는 방향이 하나뿐이라는 점이 중요하다. **`라`·`매` 를 2음절 목록에 넣으면 안 된다** —
`사라`(Sarah)·`고라`·`자매` 가 통째로 사라진다. 그래서 접미사가 아니라 **목록 전체와 정확히
일치할 때만** 버린다. `사라` 는 목록에 없으므로 멀쩡하다
(`dropTwoSyllableVerbTailExactMatch`, `twoSyllableNounEndingWithVerbTailChar`).
...
4. 후보 화면에서 **명사만 골라 `APPROVED` 로 등록**하고 나머지는 선택 차단으로 쓸어 담는다
   (`POST /words/bulk` 에 `status` 를 실어 보낸다). 빈도 조건 일괄 등록은 `CANDIDATE` 전용이라
   화면에 나오지 않는다(§3.2)
...
> ⚠️ **4번을 검수 없이 돌리면 통계가 망가진다. 2026-08-24 에 실제로 그렇게 됐다.**
>
> KRV 에 705건이 그대로 들어가 창세기 재계산 후 화면에 `하라(3)`·`하매(2)`·`몸에(2)`·`악을(2)`·
> `하였으므(1)` 같은 것이 올라왔다. `하였으므` 는 "하였으므로" 에서 조사 `로` 가 떨어져 나온
> 조각으로, 존재하지 않는 말이다.
>
> **매처가 어휘 조회를 정규화보다 먼저 하기 때문에**(§4.1 의 2·3번) 어휘에 들어간 순간
> 그것들이 모든 필터를 건너뛴다. 즉 **규칙을 고쳐도 화면이 고쳐지지 않는다.** 어휘를 지우거나
> 차단해야 한다. 이 순서는 `여자를` 을 살리기 위한 의도적 설계라 되돌릴 수 없다.
>
> 화면의 "최소 빈도" 는 **몇 번 나왔는지일 뿐 품질과 무관하다.** 활용형은 오히려 빈도가 높다.
>
> `bulkCreateCandidates` 에 방어를 붙여 두었다(정규화가 `null` 인 것 + 조사를 뗀 결과가 이미
> 어휘에 있는 것을 거부, 거부 건수를 화면에 표시). 다만 **이것으로 충분하지 않다.**
> `올라가니`·`죽으매`·`입맞추고` 처럼 어미 목록에 없는 3음절 이상 활용형은 통과한다. 705건에
> 적용했을 때 걸러지는 것은 16건뿐이었다. 규칙 기반의 한계이고, 형태소 분석기를 도입할 때까지
> **후보를 사람이 훑는 단계를 건너뛸 수 없다**(§3.2).
>
> 대안은 §3.2 의 정책 스위치다. `CANDIDATE` 를 **사용자 화면에 노출하지 않도록** 바꾸면
> 어휘를 크게 넣어 두고 승인 작업을 천천히 해도 화면이 안전하다. 규모가 커지면 이 방향이 맞다.
...
- `statuses` 는 노출 정책(§3.2)에 따라 `[APPROVED]` 또는 `[APPROVED, CANDIDATE]` 를 넘긴다.
  ...
  **기본값 `false`**, 환경변수 `WORD_STATS_INCLUDE_CANDIDATE` 로 덮어쓴다).
...
### 10.10 모바일에서는 바텀 시트로 뜬다

좁은 화면에서 이 다이얼로그는 **가운데 뜬 카드가 아니라 아래에서 올라오는 시트**다. 기준 폭은
`599.98px` 이고, 이 저장소가 이미 두 번 같은 판단을 내린 값이다(top-nav 계정 메뉴,
성경 역사 지도 상세 패널).

**데스크톱까지 바꾸지는 않는다.** 시트의 이점은 화면 아래쪽이 엄지에 가깝다는 것 하나인데,
마우스에는 그런 사정이 없다. 680px 짜리 2열 목록을 화면 바닥에 붙이면 시선만 멀어진다.

바꾼 것은 배치와 닫는 방법뿐이고, 클라우드·목록·팝오버의 역할 분담(§10.3~§10.5)은 그대로다.

| 항목 | 데스크톱 | 모바일 시트 |
|---|---|---|
| 위치 | 화면 가운데 | 아래 끝에 붙음 (`margin: auto auto 0`) |
| 폭 | `min(680px, 100vw - 24px)` | `100%` — 좌우 테두리 없음 |
| 높이 상한 | `min(82vh, 820px)` | `85dvh` |
| 모서리 | `14px` | 위쪽만 `16px` |
| 그립 | 없음 | 상단 가운데 36×4px |
| 닫는 법 | 버튼 · ESC · 배경 클릭 | + **아래로 끌어내리기** |

실측(390×844): 시트 높이 717px, 좌·우·아래 여백 0. 320×640 에서도 544px 로 같은 비율이다.
데스크톱 1280×900 은 680×738 가운데 정렬로 이전과 같다.

#### 그립은 의사 요소가 아니라 실제 `<div>` 다

`::before` 로 그리면 **그립을 누른 이벤트의 `target` 이 다이얼로그 자신이 된다.** 이 다이얼로그는
바깥 클릭을 `target === dialog` 로 판정하므로(배경 `::backdrop` 을 누르면 그렇게 들어온다),
손잡이를 살짝 건드리기만 해도 배경을 누른 것으로 오인해 시트가 닫힌다. 마크업에
`.word-stats-sheet-grip` 을 두고 데스크톱에서는 `display: none` 으로 감춘다.

#### 여는 애니메이션은 일회용 클래스로 건다 — `[open]` 에 걸면 안 된다

`.word-stats-dialog[open] { animation: word-stats-sheet-in }` 로 두면 **시트를 한 번 건드릴
때마다 화면 밖에서 다시 올라온다.** 끌어내리기를 위해 `.is-dragging` 이 `animation: none` 으로
애니메이션을 취소하는데, 손을 뗄 때 그 클래스가 빠지면서 `animation-name` 이 되살아나면
브라우저가 **애니메이션을 새로 만든다.** `[open]` 은 다이얼로그가 열려 있는 내내 참이므로
조건이 늘 맞고, 그래서 목록을 한 번 누르기만 해도 시트가 다시 슬라이드-업 한다.

실측(390×844): 그립을 탭했다 떼는 순간 `word-stats-sheet-in` 이 다시 생성되고 시트가
화면 아래 452px 지점에서 올라왔다.

그래서 여는 순간에만 `.is-opening` 을 붙이고 `animationend` 에서 뗀다. 뗀 뒤에는 되살아날
`animation-name` 자체가 없으므로 `.is-dragging` 이 붙었다 떨어져도 아무 일도 일어나지 않는다.

같은 이유로 **`.is-dragging` 은 손가락이 실제로 움직인 뒤에 붙인다.** `touchstart` 에서 바로
붙이면 그냥 누르기만 해도 클래스가 붙었다 떨어진다. `SHEET_DRAG_SLOP_PX`(6px)로 누르기와
끌기를 가른다.

#### 닫는 경로를 하나로 모은다

`dialog.close()` 는 `[open]` 을 즉시 떼어 내므로 그것만으로는 **내려가는 애니메이션을 재생할 수
없다.** 그래서 `closeDialog()` 하나로 모아, 시트일 때는 `.is-closing` 을 붙여 CSS 애니메이션을
돌린 뒤 `animationend` 에서 실제로 닫는다. 데스크톱과 `prefers-reduced-motion: reduce` 에서는
우회 없이 곧장 닫는다.

- ESC 는 `cancel` 을 `preventDefault()` 하고 같은 경로를 탄다. 팝오버가 열려 있으면 팝오버부터
  닫는 기존 규칙이 먼저다(§10.5).
- `animationend` 가 오지 않는 경우(탭 비활성 등)를 대비해 보정 타이머를 함께 건다. CSS 의
  애니메이션 시간과 JS 의 `SHEET_CLOSE_MS` 는 **같은 값이어야 한다.**

#### 끌어내리기는 본문이 맨 위일 때만 시작한다

손잡이와 헤더에서는 언제나 시작하고, 본문에서는 `scrollTop === 0` 일 때만 시작한다. 이 조건이
없으면 목록을 아래로 스크롤하려는 손짓이 매번 시트를 닫는다. 팝오버 안에서 시작한 손짓도
제외한다 — 뜻풀이를 읽으려고 스크롤하다가 시트가 닫히기 때문이다.

| 값 | 기준 | 이유 |
|---|---|---|
| 드래그 판정 | 6px | 이만큼 움직이기 전에는 '누르기'다 |
| 닫히는 거리 | 80px | 더 짧으면 스크롤하려던 손짓에도 닫힌다 |
| 닫히는 속도 | 0.5px/ms | 짧게 튕겨 내리는 동작을 살린다 |
| 튕김 유효 시간 | 120ms | 마지막 움직임이 이보다 오래됐으면 튕긴 것이 아니다 |

**`touchcancel` 은 닫지 않고 제자리로 돌린다.** `touchend` 와 묶어 두면, 시스템 제스처나
전화 수신이 손짓을 가로챘을 뿐인데 시트가 사라진다. 취소는 '손을 뗀 것'이 아니라 '없던 일'이다.

**튕김 판정에는 마지막 움직임 시각을 함께 본다.** 속도만 보면, 살짝 튕겼다가 손가락을 멈춰
붙잡고 있다 떼도 그때의 낡은 속도가 남아 그냥 닫힌다. 붙잡은 것은 취소하겠다는 뜻이다.

손을 뗀 지점은 `--sheet-drag` 로 넘겨 그 자리에서 이어서 내려가게 한다. 넘기지 않으면 0 에서
다시 시작해 위로 한 번 튄 뒤 내려간다. 이 값들과 드래그 처리 형태는 `fragments/header.html` 의
계정 메뉴 시트와 같다.

#### 안전 영역

본문 아래 여백과 팝오버 위치에 `env(safe-area-inset-bottom)` 을 더한다. 시트는 화면 맨 아래에
붙으므로, 더하지 않으면 마지막 항목과 팝오버 버튼이 홈 인디케이터에 가린다.
