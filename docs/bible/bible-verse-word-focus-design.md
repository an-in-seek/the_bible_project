...
| `templates/bible/verse-list.html` | `?v=7.3` · `?v=5.9` |
...
| `state.focusWord` · `state.focusVerseNumber` | **스포트라이트가 꺼질 때까지 산다** — 그 사이에는 같은 장을 몇 번 다시 그리든 다시 칠한다(§6.5) |
...
### 6.5 해제 시점 — 스포트라이트와 함께 사라진다

스포트라이트는 4초 뒤 또는 클릭으로 꺼진다. **단어 색도 그때 함께 지운다.**

둘은 "이 절의 이 단어를 보라" 는 **한 번의 안내**다. 스포트라이트만 걷히고 색이 남으면,
사용자에게는 지울 방법이 없는 표식이 화면에 붙는다. 형광펜과 달리 사용자가 칠한 것이
아니므로 끄는 수단을 어디에도 두지 않았고, 하단 3버튼은 이미 `flex-equal` 로 꽉 차 있어
지우는 버튼을 놓을 자리도 없다.

> 처음에는 반대로 정했다 — "오버레이가 사라진 뒤 본문을 읽는 동안이야말로 그 단어가 어디
> 있는지 보여야 할 때" 라고 봤다. 지울 수단이 없다는 쪽이 더 무겁다고 판단해 뒤집었다.

```js
// highlightVerse() 의 dismiss()
targetRow?.classList.remove("verse-spotlight-target-row");
clearFocusWord();
```

**마크만 지우는 것으로는 부족하다.** `clearFocusWord()` 는 `state.focusWord` 와
`state.focusVerseNumber` 까지 비운다. 상태를 남겨 두면 대역을 켜고 끌 때 `renderChapter` 가
다시 칠해(§6.2) **해제한 강조가 되살아난다.**

감쌌던 글자는 텍스트 노드로 되돌리고 `normalize()` 로 이웃과 다시 합친다. 합치지 않으면 절
하나가 텍스트 노드 여러 개로 쪼개진 채 남는다. 실측으로 해제 뒤 `.verse-text` 의 자식은
텍스트 노드 **하나**이고 내용은 원문과 같다.

`dismiss()` 는 캡처해 둔 참조가 아니라 `document.querySelectorAll` 로 **그 시점의 DOM** 을
훑는다. 스포트라이트가 떠 있는 4초 안에 대역을 토글하면 표가 새로 그려져 마크도 새 것이
되는데, 참조를 붙들고 있으면 사라진 노드를 지우고 새 마크는 남는다.
...
| `js/bible/verse-list.js` | `5.7` | `5.9` | `bible/verse-list.html` |
...
| 9 | 다크 테마 + 스포트라이트 4초 후 | 흰 카드 위 본문이 어둡게 나온다 (§7) |
| 9-1 | 스포트라이트 4초 자동 해제 | **강조도 함께 사라지고** 절이 원문 그대로 복원된다 (§6.5) |
| 9-2 | 오버레이 클릭으로 해제 | 위와 같다 (§6.5) |
| 9-3 | 해제한 뒤 대역 토글 | **되살아나지 않는다** — 상태까지 비웠기 때문 (§6.5) |
...
| 16 | **스포트라이트가 떠 있는 동안** 대역을 켰다 껐다 | 표를 다시 그려도 강조가 그대로 남는다 (§6.2) |
...
| **자동 해제 (9-1번)** | 0.9초·3.5초에 마크 1개, **4.9초에 0개**. 절은 텍스트 노드 하나로 복원 |
| **해제 후 대역 토글 (9-3번)** | 켜도 꺼도 마크 0개 — 되살아나지 않는다 |
