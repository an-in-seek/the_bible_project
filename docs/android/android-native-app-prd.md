...
* 부분 SPA/리치 인터랙션: Three.js 우주 배경, 게임(타이핑/퀴즈/낱말퍼즐), 정적 JS 데이터 페이지(족보, 성경 개관 영상)
...
| 성경 개관 영상 | 포함 | `bible-overview-video.js`의 66권 YouTube 데이터 → 네이티브 그리드/검색/외부 YouTube |
...
이 조합은 정합적이다. 게임·커뮤니티만 2차로 미루며, 이 둘이 바로 WebView가 필요했을 화면들이다. v1에 남는 화면은 (1) 기존 클라이언트 API가 있거나(성경/사전/마이/지원) (2) DB 의존 없는 **정적 콘텐츠**(학습 본문·족보·개관·역사 — `HistoryDummyData` 포함)여서, 둘 다 네이티브로 구현 가능하다.
...
| 학습 | 성경 개관 영상 | - (정적 JS) | 영상 목록 네이티브화(썸네일+YouTube/ExoPlayer). 이식 공수 주의 |
...
> 학습(study)은 v1 포함으로 변경됨(2026-06-30 디렉티브). 단 정적 콘텐츠 이식이라 백엔드 작업은 없으나 화면 수가 많아 **v1 공수가 늘어난다**(특히 족보·개관영상). 일정 압박 시 족보/개관영상만 후순위 스프린트로 분리하는 것을 권장(범위는 v1 유지, 내부 순서만 조정).
...
5. **학습(study) 전체 → 확정: v1 포함, 정적 데이터 네이티브 번들 방식.** 모든 학습 화면이 DB 의존 없는 정적 콘텐츠(템플릿 본문 + 정적 JS 배열 + `HistoryDummyData`)이므로 **백엔드 API 신설 없이** 텍스트/데이터를 앱 리소스(JSON/문자열)로 이식한다. WebView 미사용 원칙 유지. 족보 인터랙티브 트리와 개관 영상은 이식 공수가 크므로 v1 내 스프린트 순서를 후순위로 둘 수 있다.
...
### 4-A.7 족보·개관영상 이식 공수 및 스프린트 분리안

학습 화면 중 공수가 큰 두 항목(족보·개관영상)을 별도 산정하고, v1 범위는 유지하되 내부 스프린트를 분리해 MVP 본류를 막지 않도록 한다.

#### 소스 분석 요약

| 항목 | 소스 | 데이터 | 핵심 인터랙션 | 구조적 난도 |
|------|------|--------|---------------|-------------|
| 개관영상 | `bible-overview-video.js`(265L)/css(189L) | 66권 `{bookOrder, bookName, youtubeUrl}` | 검색 필터, 카드 탭→외부 YouTube, 딥링크 스포트라이트 | **낮음** — OT/NT 그리드 + 검색 |
...
* **S2 (학습 리치, 분리)** — **족보 + 개관영상**. 공수 큰 두 항목만 묶어 M1 후반 별도 트랙으로. 다음 규칙을 적용:
  ...
  4. **착수 우선순위** — 둘 중 **개관영상 먼저**(저난도·빠른 가치) → 족보. 개관영상에서 그리드/검색/딥링크 패턴을 먼저 확립해 재사용.
* **권장** — S2는 개관영상(2.5~3.5d) → 족보(4.5~5.5d) 순. 스포트라이트 효과는 1차에서 생략 가능(폴리시 후속).
...
* **분리 가능(S2)**: 족보+개관영상 = **8 dev-day**. 일정 압박 시 빼면 **코어 v1 ≈ 51 dev-day(버퍼 포함 ≈ 60)** 로 단축, S2는 `v1.0.x` 패스트팔로우(4-A.7 규칙).
...
* 학습 전체(사전 + 정적 콘텐츠 + 족보 + 개관영상 + 성경역사) — 정적 데이터 번들로 네이티브화(N). 족보/개관영상은 별도 트랙 S2로 분리(공수·분리안 4-A.7 참조).
...
| 족보·개관영상 이식 공수 | v1 일정 압박 | S2로 분리, 필요 시 `v1.0.x` 패스트팔로우(4-A.7) |
...
| 개관영상 | `src/main/resources/static/js/study/bible-overview-video.js` (66권 데이터) + 동명 css/html | 영상 목록·검색·딥링크 사양 |
