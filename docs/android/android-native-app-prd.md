...
> ### ⚠️ 이 문서는 현행 구현과 다릅니다 (superseded)
>
> 본 PRD는 v1을 **"전부 네이티브, WebView 미사용"**(§3, §4-A)으로 결정했으나,
> **실제 출시된 Android 앱(`el-seeker-android`)은 WebView 하이브리드 셸로 구현**되었습니다.
> iOS 앱(`el-seeker-ios`)도 동일하게 WebView 하이브리드입니다.
>
> 현행 구조의 정본은 `el-seeker-android` 의 `CLAUDE.md` 와
> `docs/android-app-architecture.md` 입니다. 본 문서는 당시의 검토·공수 산정 기록으로만
> 참고하고, 구현 방침의 근거로 인용하지 마세요.

상태: **기획 단계 (구현 전) — 이후 다른 방식으로 구현되어 폐기됨**
