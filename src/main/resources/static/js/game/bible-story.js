        ...
        beats: [
            {
                ...
                lines: [
                    ...
                    {who: '말씀', text: '보라 세상 죄를 지고 가는 하나님의 어린 양이로다', ref: '요한복음 1:29'},
                    {who: '기록자', text: '어린 양이라니. 나는 낡은 두루마리에서 네 개의 조각을 꺼냈다. 이 연결은 내 손으로 확인해야 한다.'}
                ]
            },
            {
                type: 'match',
            ...
            },
            {
                type: 'say',
                lines: [
                    {text: '그리고 그 어린 양이라 불리신 예수께서는 유월절 저녁, 제자들과 마지막 식탁에 앉아 떡과 잔을 나누셨다.', ref: '누가복음 22:14–20'}
                ]
            }
        ]
    },
...
function setupListeners() {
    ...
    elements.storySheet.addEventListener('click', (event) => {
        if (event.target === elements.storySheet) closeSheet();
    });
    ...
    window.addEventListener('resize', () => {
        if (redrawMatchLines) redrawMatchLines();
    });
}
...
function startNewStory() {
    if (hasSavedProgress() && !window.confirm('기존 진행 기록을 지우고 새로 시작할까요?')) return;
...
function renderLine(line, onAdvance, hint = '탭하여 계속') {
    if (line.phase) elements.storyBackdrop.dataset.phase = line.phase;
...
function renderDoorBeat(beat) {
    ...
    elements.storyPlayArea.querySelectorAll('.story-door-spot').forEach((spot) => {
        spot.addEventListener('click', () => {
            if (spot.classList.contains('is-marked')) return;
    ...
    const runRound = () => {
        ...
        MANNA_SPOTS.forEach(([x, y]) => {
            ...
            piece.addEventListener('click', () => {
                if (piece.classList.contains('is-picked')) return;
        ...
        doneButton.addEventListener('click', () => {
            ...
            if (over && !lastRound) state.mannaGreed = true;
...
function renderMatchBeat(beat) {
    ...
    elements.storyPlayArea.innerHTML = `
        <div class="story-match-board" id="matchBoard">
            <svg class="story-match-lines" id="matchLines" aria-hidden="true"></svg>
    ...
    const svg = elements.storyPlayArea.querySelector('#matchLines');
    const fragmentsCol = elements.storyPlayArea.querySelector('#matchFragments');
    const testimoniesCol = elements.storyPlayArea.querySelector('#matchTestimonies');
    ...
    const drawLines = () => {
        ...
        svg.innerHTML = pairs.map(({from, to}) => {
    ...
    Object.entries(FRAGMENTS).forEach(([id, fragment]) => {
        ...
        button.innerHTML = `<span class="story-fragment-gem" aria-hidden="true"></span><strong>${fragment.title}</strong>`;
        button.setAttribute('aria-pressed', 'false');
        button.addEventListener('click', () => {
            if (button.classList.contains('is-linked')) return;
            fragmentsCol.querySelectorAll('.story-match-card').forEach((card) => {
    ...
    TESTIMONIES.forEach((testimony) => {
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'story-match-card story-match-testimony';
        button.dataset.matchId = testimony.id;
        button.innerHTML = `<span class="story-match-quote">“${testimony.text}”</span><small>${testimony.ref}</small>`;
        button.addEventListener('click', () => {
            if (button.classList.contains('is-linked')) return;
            if (!selected) {
            ...
            if (selected.dataset.matchId === testimony.id) {
                ...
                selected.classList.add('is-linked');
                button.classList.add('is-linked');
                pairs.push({from: selected, to: button});
                drawLines();
                announce(`${FRAGMENTS[testimony.id].title} — ${testimony.ref} 연결.`);
                note.textContent = `이어졌습니다. (${pairs.length} / ${TESTIMONIES.length})`;
                selected = null;
                if (pairs.length === TESTIMONIES.length) finishMatch(beat, board, note);
            } else {
...
function renderFragmentBeat(scene, beat) {
    ...
    elements.storyPlayArea.querySelector('#fragmentTakeButton').addEventListener('click', () => {
        if (!state.fragments.includes(beat.id)) state.fragments.push(beat.id);
...
function openSheet() {
    renderSheet();
    if (!elements.storySheet.open) elements.storySheet.showModal();
}

function closeSheet() {
    if (elements.storySheet.open) elements.storySheet.close();
}
...
function confirmRestart() {
    if (!window.confirm('현재 기록을 지우고 처음부터 다시 시작할까요?')) return;
    closeSheet();
    state = createInitialState();
    saveState();
    updateFragmentCount(false);
