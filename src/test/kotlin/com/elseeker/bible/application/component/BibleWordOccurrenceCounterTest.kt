...
class BibleWordOccurrenceCounterTest {
    ...
    fun countsEveryOccurrence() {
        // given
        val verses = listOf(verse(1, "하나님이 하나님을 하나님과"))
    ...
    fun countsMultiWordTerm() {
        // given
        val verses = listOf(verse(1, "하나님 나라가 가까이 왔으니 하나님 나라를 구하라"))
    ...
    fun countsWithoutWhitespace() {
        // given
        val verses = listOf(verse(1, "起初神創造天地神說"))
    ...
    fun countsAliasWithoutDoubleCounting() {
        ...
        val verses = listOf(verse(1, "하나님 아버지께 하나님이"))
    ...
    fun ignoresCaseForLatin() {
        // given
        val verses = listOf(verse(1, "God created. In GOD we trust, god."))
    ...
    fun countsSubstringInsideOtherWord() {
        // given
        val verses = listOf(verse(1, "말씀이 계시니라 말미암아"))
    ...
    @Test
    @DisplayName("한 번도 나오지 않으면 0 회다")
    fun countsNothing() {
        // given
        val verses = listOf(verse(1, "태초에 천지를 창조하시니라"))
        ...
        counted.chapterCounts.isEmpty() shouldBe true
    }

    private fun verse(chapterNumber: Int, text: String) =
        ChapterVerseText(chapterNumber = chapterNumber, text = text)
}
